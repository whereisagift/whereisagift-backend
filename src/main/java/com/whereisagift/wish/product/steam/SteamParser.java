...
public class SteamParser implements ProductParser {
  ...
  public Product parse(String url) {
    String appId =
        extractAppId(url)
            .orElseThrow(
                () -> new IllegalArgumentException("Cannot extract Steam app ID from URL: " + url));
  ...
  private JsonNode fetchAppDetails(String appId) {
    URI uri =
        UriComponentsBuilder.fromUriString(BASE_API_URL)
    ...
    try {
      ...
      if (!appNode.path("success").asBoolean(false)) {
        throw new IllegalStateException("Steam API returned success=false for appId=" + appId);
      }
      return appNode;
    } catch (Exception ex) {
  ...
  private Optional<Price> parsePrice(JsonNode info) {
    ...
    BigDecimal finalPrice = BigDecimal.valueOf(priceNode.path("final").asLong(), 2);
